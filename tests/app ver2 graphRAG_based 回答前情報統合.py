            ...
            with st.spinner("Generating answer..."):
                try:
                    functions = [
                        ...
                        {
                            "name": "answer_based_on_information",
                            "description": "クエリに対して、データに基づいて回答可能かを判定し、キーワードを返します。",
                            "parameters": {
                                "type": "object",
                                "properties": {
                                    "answer": {"type": "string", "description": "回答可能か回答不能かを返します。"},
                                    "keywords": {"type": "array", "items": {"type": "string"}, "description": "キーワードのリストを返します。"}
                                },
                    ...
                    extraction_response = llm([
                        {"role": "system", "content": """あなたは検索のプロフェッショナルです。次の質問から固有名詞または特徴的な単語を、複合語に注意して抽出してください。
                         複合語が含まれている場合、名詞が2つ以上連結されている箇所を分割し、それぞれの名詞を独立した単語として抽出してください。
                         例えば、「研究グループ」は意味の独立した単語の複合語なので「研究」と「グループ」に分けますが、「鶴の一声」のような慣用句については分けると意味が損なわれるので「鶴の一声」のままとします。また、「メンバー」「科」などの一般的な名詞が含まれている場合は、他の名詞と分割します。"""},
                    ...
                    if 'selected_file' in locals():
                        ...
                        if metadata_response_json["Sufficiency"] == "不十分":
                            ...
                            if 'keyword_hits' in locals() and keyword_hits is not None and any(keyword_hits.values()):
                                ...
                                messages = [
                                    {"role": "system", "content": """
                                    質問に対して、引用箇所をセットにした回答を作成することが目的です。
                                    与えられた”単語とその登場回数”と”グラフDB”を読み解き、回答に関する情報が含まれる場合は「回答可能」と返し、”グラフDB”の内容を参考に引用箇所を探すためのキーワードをリスト化してください。
                                    - 人物名や組織名などの固有名詞はヒット数が増えすぎてしまうので極力含めません。
                                    回答に関する情報が含まれない場合は「回答不能」とだけ返してください。
                                    """},
                                    {"role": "user", "content": f"質問: {query}\n\n#単語とその登場回数:{keyword_hits_str}\n\n#グラフDB:\n{neo4j_data}"}
                                ]
                                ...
                                llm = ChatOpenAI(
                                    ...
                                    model="gpt-4o",
                                    functions=functions,
                                    function_call={"name": "answer_based_on_information"}
                                )
                                ...
                                # 「回答可能」の場合、quote_keyword にキーワードを格納
                                quote_keywords = []
                                extracted_sentences = []
                                if short_answer == "回答可能":
                                    quote_keywords = arguments.get('keywords', [])

                                    # ヒットしたチャンクからキーワードが含まれる文を取得する処理
                                    def extract_sentences_with_keywords(chunk, keywords):
                                        sentences_with_keywords = []
                                        sentences = chunk.page_content.split('。')  # 文単位で分割
                                        for sentence in sentences:
                                            for keyword in keywords:
                                                if keyword in sentence:
                                                    sentences_with_keywords.append(sentence)
                                                    break  # 一文に複数のキーワードが含まれても、一度に一文だけ追加
                                        return sentences_with_keywords

                                    # ヒットしたチャンクに対してキーワードが含まれる文をリスト形式で取得
                                    for chunk in matched_chunks:
                                        sentences = extract_sentences_with_keywords(chunk, quote_keywords)
                                        extracted_sentences.extend(sentences)
                                    
                                    # 重複を排除してリストを作成
                                    unique_sentences = list(set(extracted_sentences))

                                    # 取得した文のリストを表示（または他の処理に利用）
                                    st.write(unique_sentences)
                            ...
                            if filtered_keywords and any(keyword_hits.values()):
                                ...
                                # 上位のチャンクを取得
                                docs = [doc for doc, score in sorted_chunks][:3]
                                st.write(docs)
                            ...
                            if unique_sentences:
                                ...
                                messages = [
                                    {"role": "system", "content": """
                                    質問に関係のある文章を抜けもれなくすべて抜き出して出力してください。まだ回答は考えません。
                                    - 文書全体を調べた結果判明した”一致箇所”を確認し、その内容を文脈と合わせて理解します。"""},
                                    {"role": "user", "content": f"質問: {query}\n\n#一致箇所\n{unique_sentences}\n\n#文脈:\n{context}"}
                                ]
                                ...
                                messages = [
                                    ...
                                    ②その他の質問の場合、質問に対して厳密（”～以上”などの細かい表現に注意する）かつ簡潔な回答（50字以内）を作成し、その根拠を引用してください。
                            ...
                            if unique_sentences:
                                ...
                                messages = [
                                    {"role": "system", "content": """
                                    ①「総額」や「いくら？」などの計算に関する質問の場合、自分で計算を行わなわず、必ずmultiply関数を呼び出してください。
                                    ②その他の質問の場合、質問に対して簡潔な回答（50字以内）を作成し、その根拠を引用してください。
                                    - 必ず「～以上」などの細かい表現に注意します
                                    - 文書全体を調べた結果判明した関連する”引用文”を確認し、その内容を文脈と合わせて理解します。
                                    - 論理的に明らかに質問の前提に誤りがあると判断される場合は「質問誤り」と答えます。"""},
                                    {"role": "user", "content": f"質問: {query}\n\n#引用文\n{response}"}
                                ]
                            else:  # keyword_hitsが存在しないか、空の場合
