...
with st.sidebar:
    ...
    model_name = st.text_input("Enter model name", value=st.session_state.model_name)
...
if st.session_state.mode == 'RAG' and (st.session_state.document_processed or st.session_state.data_loaded):
            ...
            with st.spinner("Generating answer..."):
                try:
                    ...
                    if specified_files:
                        ...
                        st.write(f"選択されたファイル: {', '.join(specified_files)}")

                        # 指定されたすべてのファイルからチャンクを取得
                        for specified_file in specified_files:
                            st.write(f"処理中のファイル: {specified_file}")
                            for doc in st.session_state.split_docs.get(specified_file, []):
                                matched_chunks.append(doc)

                        relevant_texts = []  # 関連する文の塊や一文を格納するリスト

                    else:
                        st.write("ファイル名がクエリに含まれていません。キーワード検索を行います。")
                        ...
                        if extracted_keywords:
                            ...
                            # ヒットしたすべてのファイルを処理
                            if matched_files:
                                specified_files = list(matched_files)  # セットをリストに変換
                                st.write(f"選択されたファイル: {', '.join(specified_files)}")
                            ...
                            st.error("キーワードが抽出されませんでした。")
                            matched_chunks = []
                            specified_files = []

                    # 関連する文の抽出
                    if specified_files and matched_chunks:
                        relevant_texts = []  # 関連する文の塊や一文を格納するリスト

                        for doc in matched_chunks:
                            # プロンプトの作成
                            prompt = f"""質問を回答する材料を漏れなく集めます。関連する材料が含まれる文章を、周辺部含めてドキュメントのチャンクから”そのまま抜き出して”ください。複数ある場合は配列に分け、ない場合は空配列を返します。回答は内容のみで、マークダウンを含めないでください。
    ...
    """
                            ...
                            response = llm(prompt)
                            st.write(f"LLM応答: {response.content}")

                            # 応答がAIMessageオブジェクトの場合、その内容を取得
                            if isinstance(response, AIMessage):
                                response_content = response.content  # AIMessageオブジェクトからコンテンツ（文字列）を取得
                            else:
                                response_content = response  # 既に文字列の場合

                            # 応答をリストとして解釈し、relevant_textsに追加
                            try:
                                # 応答がJSON形式のリストであることを期待
                                extracted_texts = json.loads(response_content)
                                if isinstance(extracted_texts, list):
                                    relevant_texts.extend(extracted_texts)
                                else:
                                    # リストでない場合はそのまま追加
                                    relevant_texts.append(response_content.strip())
                            except json.JSONDecodeError:
                                # JSONとして解釈できない場合はそのまま追加
                                if response_content.strip():
                                    relevant_texts.append(response_content.strip())
                            except Exception as e:
                                st.error(f"チャンクの処理中にエラーが発生しました: {str(e)}")

                        # 重複を削除（オーバーラップによる同一文章を除外）
                        unique_texts = []
                        seen_texts = set()
                        ...
                        relevant_texts = unique_texts

                        # 関連する文が見つかったか確認
                        if relevant_texts:
                            st.write("関連する文が見つかりました。")

                            # 回答の生成（Function Calling）
                            messages = [
                                {"role": "system", "content": """
                                    質問に対して厳密（”～以上”などの細かい表現に注意する）かつ簡潔な回答（50字以内）を作成し、その根拠を引用してください。
                                    - 文書全体を調べた結果判明した”関連する文の塊や一文”を確認し、その内容を文脈と合わせて理解します。
                                    - 論理的に明らかに質問の前提に誤りがあると判断される場合は「質問誤り」と答えます。"""},
                                {"role": "user", "content": f"質問: {query}\n関連する文の塊や一文: {json.dumps(relevant_texts, ensure_ascii=False)}"}
                            ]
                            ...
                            st.subheader("根拠となる引用:")
                            st.write(supporting_quote)
                        else:
                            st.error("関連する文が見つかりませんでした。")
                    else:
                        st.error("指定されたファイルが見つからないか、関連するチャンクが存在しません。")
